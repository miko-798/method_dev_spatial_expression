...
# gold_stand_list: list containing all the matrices for cell type gold standard
# ct_list: a vector list of all cell type names
norm_E_simulate <- function(gene_of_choice, spatial, n_spot, n_ct, gold_stand_list, ct_list){

  # initialization
  norm.E <- matrix(NA, nrow = n_spot, ncol = n_ct)
  colnames(norm.E) <- ct_list
  rownames(norm.E) <- rownames(spatial)
  
  counter = 1
  for(mat in gold_stand_list){
    print(dim(mat))
    rows <- rownames(norm.E)[rownames(norm.E) %in% rownames(mat)]
    norm.E[rows,counter] <- mat[rows, gene_of_choice]
...
min.obj.fn <- function(exp_spot_celltype, lambda, n, m, n_spot, n_ct){
  ...
  #predicted <- log2(rowSums(decon_input*(2^input_exp-1))+1)
